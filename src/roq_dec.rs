...
/// Trait which handles decoding YCbCr into arbitrary color format for display
pub trait Colorspace<TColor>: where TColor : Sized + Clone + Copy {
    fn default() -> TColor;
    fn convert(y: i32, cb: i32, cr: i32, a: u8) -> TColor;
}

/// Represents a 16 bit-per-pixel 3 channel BGR color format
...
/// Enumeration of errors which can result from trying to decode a ROQ stream
#[derive(Debug)]
pub enum RoqError {
    IOError(Error),
    ParseError,
...
/// Enumeration of types of data which can be received from a RoqDecoder
pub enum RoqEvent<'a, C> where C:Sized+Clone+Copy {
    InitVideo,
    Audio (i32, &'a[i16]),
    Video (&'a[C]),
    Custom (u16, u16, Vec<u8>),
    EndOfFile,
}

/// Represents a decoder which can be constructed from a ROQ video stream and return chunks of audio/video information from that stream
pub struct RoqDecoder<R,C,S>
...
impl<R,C,S> RoqDecoder<R,C,S> where R : Read + Seek, C:Sized + Clone + Copy, S : Colorspace<C> {
    pub fn new(mut reader: R) -> Result<RoqDecoder<R,C,S>,RoqError> {
        // read ROQ header
    ...
    fn get_framebuffer_data(&self) -> &[C] {
                    ...
                    match self.get_mode(reader, &mut mode_count, &mut mode_set)? {
                        ...
                        1 => {
                            ...
                            if dposx < 0 || dposx > (self.width - 8) as i32 ||
                                ...
                                println!("WARNING: Motion vector out of bounds: Pos = ({}, {}), MV = ({}, {}), Bounds = ({}, {})",
                                    posx, posy,
                                    dposx, dposy,
            ...
            match chunk_id {
                ...
                0x1020 => {
                    ...
                    for i in 0..sample_cnt {
                        snd = snd.saturating_add(SND_SQR_LUT[chunk_reader[i] as usize]);
                        self.audiobuf[i] = snd;
                    }
                ...
                0x1021 => {
                    ...
                    for i in 0..sample_cnt {
                        snd_left = snd_left.saturating_add(SND_SQR_LUT[chunk_reader[i * 2] as usize]);
                        snd_right = snd_right.saturating_add(SND_SQR_LUT[chunk_reader[i * 2 + 1] as usize]);
                ...
                _ => {
                    // some other chunk type - return a Custom event in case user wants to handle it
                    let mut custom_chunk_data: Vec<u8> = vec![0;chunk_data.len()];
                    custom_chunk_data.extend_from_slice(&chunk_data);
                    return Ok(RoqEvent::Custom(chunk_id, chunk_arg, custom_chunk_data));
                }
            }
        }
